/**
 * DEMO MODE — Chrome Extension content script
 * Intercepts all fetch calls and returns realistic fake data for video recording.
 *
...
window.fetch = function() {
  ...
  try {
    ...
    else if (input != null) u = String(input);
  } catch (e) { return _origFetch.apply(window, args); }

  if (!u || !u.includes('/api/')) return _origFetch.apply(window, args);
  ...
  // ── /api/reviews ─────────────────────────────────────────────────
  if (u.includes('/api/reviews') && (!args[1] || args[1].method !== 'POST')) {
...
console.log('To disable: toggle off the extension in chrome://extensions');
})();
