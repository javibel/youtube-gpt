...
const FAQ_ES = [
  { q: '¿Como funciona el A/B testing en YouTube?', a: 'Escribes dos variaciones de titulo para un video. YTubViral las rota automaticamente a intervalos fijos (ej. cada 24 horas) y registra impresiones, CTR y vistas de cada version. Cuando hay suficientes datos, declaramos un ganador estadisticamente significativo.' },
  { q: '¿Puedo hacer A/B test de thumbnails tambien?', a: 'YouTube ya tiene test nativo de thumbnails (Test & Compare). Nuestra herramienta se centra en el test de titulos, que YouTube NO ofrece de forma nativa. Combina ambos: testea thumbnails con YouTube, testea titulos con YTubViral.' },
  { q: '¿VidIQ ofrece A/B testing?', a: 'No. VidIQ no tiene A/B testing de titulos. TubeBuddy lo ofrece en su plan Legend ($49/mes). YTubViral lo incluye en Pro a 9,99 euros/mes — un 80% mas barato.' },
  { q: '¿Cuanto tiempo necesita un test?', a: 'Normalmente 7-14 dias para obtener resultados estadisticamente significativos, dependiendo del volumen de vistas de tu video. La herramienta te avisa cuando hay suficientes datos para elegir un ganador con confianza.' },
  { q: '¿El A/B testing perjudica el rendimiento de mi video?', a: 'No. Los cambios de titulo no resetean el algoritmo de YouTube ni afectan las recomendaciones. YouTube lo trata como una actualizacion de metadatos. Tu video conserva todas sus impresiones, likes y comentarios.' },
  { q: '¿Cambia los titulos en YouTube automaticamente?', a: 'Si. Con YouTube OAuth conectado, YTubViral rota los titulos automaticamente via la API. Lo configuras una vez y revisas los resultados despues. Sin cambios manuales.' },
];
...
export default async function AbTestingFeature() {
  ...
  const steps = [
    { step: '1', title: t('Elige un video y escribe dos titulos', 'Pick a video and write two titles'), desc: t('Selecciona cualquier video de tu canal. Pon tu titulo actual como version A y tu alternativa como version B. Nuestra IA puede sugerirte la version B si necesitas ideas.', 'Select any video from your channel. Write your current title as version A and your alternative as version B. Our AI can suggest version B if you need ideas.') },
    { step: '2', title: t('Configura la rotacion', 'Set the rotation schedule'), desc: t('Elige cada cuanto rotar titulos (cada 12h, 24h o 48h). La herramienta rota los titulos automaticamente via la API de YouTube — sin trabajo manual.', 'Choose how often to swap titles (every 12h, 24h, or 48h). The tool rotates titles automatically via YouTube API — no manual work.') },
    { step: '3', title: t('Dejalo correr', 'Let it run'), desc: t('YTubViral registra impresiones, CTR y velocidad de vistas para cada version del titulo. El dashboard muestra la comparativa de rendimiento en tiempo real.', 'YTubViral tracks impressions, CTR, and view velocity for each title version. The dashboard shows real-time performance comparison.') },
    { step: '4', title: t('Elige el ganador', 'Pick the winner'), desc: t('Cuando hay suficientes datos (normalmente 7-14 dias), la herramienta declara un ganador estadisticamente significativo. Aplicalo con un clic.', 'Once we have enough data (usually 7-14 days), the tool declares a statistically significant winner. Apply it with one click.') },
  ];

  const comparison = [
    [t('A/B test de titulos', 'Title A/B testing'), '✓', t('✓ (solo Legend)', '✓ (Legend only)'), '✗', '✗'],
    [t('A/B test de thumbnails', 'Thumbnail A/B testing'), '✗', t('✓ (solo Legend)', '✓ (Legend only)'), '✗', t('✓ (nativo)', '✓ (native)')],
    [t('Rotacion automatica', 'Automatic rotation'), '✓', '✓', '\u2014', '✓'],
    [t('Significancia estadistica', 'Statistical significance'), '✓', '✓', '\u2014', '✓'],
    [t('Sugerencias de titulos con IA', 'AI title suggestions'), '✓', '✗', '✓', '✗'],
    [t('Plan minimo necesario', 'Min plan required'), 'Pro (\u20ac9.99)', 'Legend ($49)', 'N/A', t('Gratis', 'Free')],
  ];
    ...
    <div className="min-h-screen" style={{ background: 'var(--ink)' }}>
      <section className="max-w-5xl mx-auto px-6 pt-24 pb-16 text-center">
        ...
        <h1 className="font-display font-bold text-4xl md:text-6xl leading-tight mb-6">
          ...
          <span style={{ color: 'var(--red)' }}>{t('titulos de YouTube', 'YouTube Titles')}</span>
        </h1>
        <p className="text-lg md:text-xl text-zinc-400 max-w-2xl mx-auto mb-8">
          {t('Deja de adivinar que titulo consigue mas clics. Configura un A/B test en 30 segundos, dejalo correr automaticamente y obtendras resultados basados en datos. La funcion que VidIQ no tiene.',
      ...
      <section className="max-w-4xl mx-auto px-6 py-16">
        <h2 className="font-display font-bold text-2xl md:text-3xl mb-6 text-center">
          {t('Por que el A/B testing de titulos importa mas de lo que crees', 'Why A/B testing titles matters more than you think')}
        </h2>
        <div className="soft-card p-6 md:p-8 space-y-4 text-zinc-400 text-[15px] leading-relaxed">
          <p>
            {t('Tu titulo es el factor #1 en si alguien hace clic en tu video. Una mejora del 1% en CTR en un video con 100.000 impresiones = ', 'Your title is the #1 factor in whether someone clicks your video. A 1% CTR improvement on a video with 100,000 impressions = ')}
            <strong className="text-white">{t('1.000 clics extra', '1,000 extra clicks')}</strong>
            {t('. Multiplica eso por tu catalogo y la diferencia es brutal.', '. Multiply that across your catalog and the difference is massive.')}
          </p>
          <p>
            {t('La mayoria de creadores eligen titulos por instinto. El top 1% los testea con datos. Hasta ahora, la unica forma de hacer esto era TubeBuddy Legend a $49/mes. YTubViral trae la misma funcionalidad a una fraccion del precio.',
          ...
          <p>
            {t('Combinado con el test nativo de thumbnails de YouTube, ahora puedes optimizar ambos elementos que impulsan el CTR — y YouTube recompensa un CTR mas alto con mas impresiones. Es un efecto compuesto.',
      ...
      <section className="max-w-4xl mx-auto px-6 py-16">
        <h2 className="font-display font-bold text-2xl md:text-3xl mb-8 text-center">
          {t('A/B testing: ¿quien lo ofrece?', 'A/B testing: who offers it?')}
        </h2>
      ...
      <section className="max-w-3xl mx-auto px-6 py-20 text-center">
        ...
        <p className="text-zinc-400 mb-8">{t('Testea tus titulos de YouTube por 9,99 euros/mes. VidIQ no puede hacer esto a ningun precio.', 'A/B test your YouTube titles for \u20ac9.99/month. VidIQ can\'t do this at any price.')}</p>
