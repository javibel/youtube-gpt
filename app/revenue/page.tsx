    ...
    return (
      <div className="min-h-screen grain flex items-center justify-center" style={{ background: 'var(--ink)', color: 'var(--text)' }}>
        <div className="text-center">
          ...
          <p className="font-mono-jb text-sm text-zinc-500">{t('Estimando ingresos...', 'Estimating revenue...')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen grain flex items-center justify-center" style={{ background: 'var(--ink)', color: 'var(--text)' }}>
        <div className="text-center">
          ...
          <p className="text-zinc-500 mb-6 font-mono-jb text-sm">{t('Inicia sesión para ver tus estimaciones.', 'Sign in to view your estimates.')}</p>
      ...
      <div className="border-b border-white/10" style={{ background: '#0B0B0D' }}>
        <div className="yv-page">
          ...
          {data && !data.hasRealRevenue && (
            <p className="font-mono-jb text-[11px] text-zinc-500 mt-2">
          ...
          {data?.hasRealRevenue && (
            <p className="font-mono-jb text-[11px] text-green-400/80 mt-2">
      ...
      <div className="yv-page">
        ...
        {data && (
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
              {[
                ...
                <div key={i} className="rounded-xl border border-white/8 p-4" style={{ background: 'rgba(255,255,255,0.02)' }}>
                  <p className="font-mono-jb text-[10px] text-zinc-500 uppercase tracking-wider mb-1">{card.label}</p>
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-8">
                ...
                <div className="space-y-2">
                  {data.countries.slice(0, 12).map(c => (
                    <div key={c.country} className="flex items-center gap-2">
                      ...
                      <span className="font-mono-jb text-[10px] text-zinc-600 w-14 text-right">${c.cpm} CPM</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="rounded-xl border border-white/8 p-5" style={{ background: 'rgba(255,255,255,0.02)' }}>
                ...
                {data.months.length > 0 ? (
                    ...
                    {data.months.map(m => (
                      <div key={m.month}>
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-mono-jb text-[11px] text-zinc-400">{m.month}</span>
                          <div className="flex items-center gap-3">
                            <span className="font-mono-jb text-[10px] text-zinc-600">{fmtNum(m.views)} views</span>
                  ...
                  <p className="text-zinc-600 font-mono-jb text-sm">{t('Sin datos mensuales suficientes', 'Not enough monthly data')}</p>
                )}
              </div>
            </div>

            {/* Top videos by revenue */}
            <div className="rounded-xl border border-white/8 p-5 mb-8" style={{ background: 'rgba(255,255,255,0.02)' }}>
              ...
              <div className="space-y-2">
                {data.videos.map((v, i) => (
                  <div key={v.videoId} className="flex items-center gap-3 group">
                    <span className="font-mono-jb text-[11px] text-zinc-600 w-5 text-right">{i + 1}</span>
                    {v.thumbnail && (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-0.5">
                        <span className="font-mono-jb text-[10px] text-zinc-600">{fmtNum(v.views)} views</span>
                        <span className="font-mono-jb text-[10px] text-zinc-600">{(v.watchTime / 60).toFixed(0)}h watch</span>
                      </div>
                    </div>
            ...
            {/* AI Insights */}
            {data.aiInsights && (
              <div className="rounded-xl border p-5 mb-8" style={{ borderColor: 'rgba(168,85,247,0.2)', background: 'rgba(168,85,247,0.04)' }}>
                ...
                <div className="grid md:grid-cols-2 gap-4 mb-4">
                  <div className="rounded-lg border border-white/8 p-3" style={{ background: 'rgba(255,255,255,0.02)' }}>
                    <p className="font-mono-jb text-[10px] text-zinc-500 uppercase tracking-wider mb-1">{t('Potencial', 'Potential')}</p>
                    <p className="font-mono-jb text-[12px] text-white leading-relaxed">{data.aiInsights.optimizationPotential}</p>
                  </div>
                  <div className="rounded-lg border border-white/8 p-3" style={{ background: 'rgba(255,255,255,0.02)' }}>
                    <p className="font-mono-jb text-[10px] text-zinc-500 uppercase tracking-wider mb-1">{t('Ingresos que dejas de ganar', 'Revenue left on the table')}</p>
                    <p className="font-mono-jb text-[14px] text-yellow-400 font-bold">{data.aiInsights.missedRevenue}</p>
                  </div>
                </div>

                <div className="mb-4">
                  <p className="font-mono-jb text-[10px] text-zinc-500 uppercase tracking-wider mb-2">{t('Estrategia CPM', 'CPM Strategy')}</p>
                  <p className="font-mono-jb text-[12px] text-purple-300/80 leading-relaxed">{data.aiInsights.cpmStrategy}</p>
                </div>

                <div>
                  <p className="font-mono-jb text-[10px] text-zinc-500 uppercase tracking-wider mb-2">{t('Tips de monetización', 'Monetization Tips')}</p>
                  <div className="space-y-1.5">
                    {data.aiInsights.tips.map((tip, i) => (
                      <div key={i} className="flex items-start gap-2">
                        <span className="font-mono-jb text-[10px] text-green-400 mt-0.5">{i + 1}.</span>
                        <p className="font-mono-jb text-[11px] text-zinc-300 leading-relaxed">{tip}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                <div key={i} className="rounded-lg border border-white/6 p-3 text-center" style={{ background: 'rgba(255,255,255,0.015)' }}>
                  <p className="font-mono-jb text-[10px] text-zinc-600 uppercase tracking-wider">{s.label}</p>
