...
function Stars({ rating }: { rating: number }) {
  return (
    <div className="flex gap-0.5">
      {[1,2,3,4,5].map(s => (
        <FaStar key={s} size={12} style={{ color: s <= rating ? '#facc15' : 'rgba(255,255,255,0.12)' }} />
      ))}
    </div>
  );
}
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated') {
      fetch('/api/admin/stats').then(r => r.json()).then(d => { if (d.error) setError(d.error); else setData(d); }).finally(() => setLoading(false));
      fetch('/api/admin/reviews').then(r => r.json()).then(d => { if (d.reviews) setReviews(d.reviews); }).finally(() => setReviewsLoading(false));
      fetch('/api/admin/feedback').then(r => r.json()).then(d => { if (d.feedbacks) { setFeedbacks(d.feedbacks); setFeedbackPending(d.pending ?? 0); } }).finally(() => setFeedbacksLoading(false));
    }
  ...
  async function handleCreateUser(e: React.FormEvent) {
    ...
    try {
      ...
      else setCreateMsg({ ok: false, text: d.error ?? 'Error al crear usuario' });
  ...
  async function handleGrantPro(e: React.FormEvent) {
    ...
    try {
      ...
      const d = await res.json();
      if (d.ok) { setGrantMsg({ ok: true, text: `Plan Pro activado para ${grantEmail}.` }); setGrantEmail(''); fetch('/api/admin/stats').then(r => r.json()).then(d => { if (!d.error) setData(d); }); }
      else setGrantMsg({ ok: false, text: d.error ?? 'Error al activar Pro' });
  ...
  async function handleModerateReview(id: string, status: 'approved' | 'rejected') {
    ...
    try {
      ...
      const d = await res.json();
      if (d.ok) setReviews(prev => prev.map(r => r.id === id ? { ...r, status } : r));
  ...
  if (error) return <div className="min-h-screen flex items-center justify-center" style={{ background: '#06060f' }}><p className="text-red-400">{error}</p></div>;
  if (!data) return null;
      ...
      <div className="relative" style={{ zIndex: 1 }}>
        ...
        <header className="sticky top-0 z-50 px-6 py-4 backdrop-blur-xl" style={{ borderBottom: '1px solid rgba(0,217,255,0.08)', background: 'rgba(6,6,15,0.85)' }}>
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
              <a href="/admin/social" className="text-xs font-semibold px-3 py-1.5 rounded-lg transition" style={{ background: 'rgba(255,0,255,0.08)', border: '1px solid rgba(255,0,255,0.2)', color: '#FF00FF' }}>Agente Social →</a>
        ...
        <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: 'Usuarios totales', value: overview.totalUsers, sub: `${overview.freeUsers} free · ${overview.proUsers} pro`, icon: <FaUsers size={16} />, color: '#00D9FF' },
              { label: 'MRR', value: `${overview.mrr.toFixed(2).replace('.', ',')}€`, sub: `${overview.proUsers} suscriptores Pro`, icon: <FaCrown size={16} />, color: '#f97316' },
              { label: 'Conversión', value: `${overview.conversionRate.toFixed(1)}%`, sub: 'Free → Pro', icon: <FaArrowUp size={16} />, color: '#a855f7' },
              { label: 'Generaciones totales', value: overview.totalGenerations.toLocaleString('es-ES'), sub: `${thisMonth.generations} este mes`, icon: <FaBolt size={16} />, color: '#00D9FF' },
            ].map((card, i) => (
              <div key={i} className="neon-card rounded-2xl p-5">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center mb-3" style={{ background: `${card.color}18`, color: card.color }}>{card.icon}</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="flex items-end gap-2 h-28">
                {daily.map(d => (
                  <div key={d.date} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    <div className="w-full rounded-t-md transition-all" style={{ height: `${Math.max((d.count / maxDaily) * 80, d.count > 0 ? 4 : 2)}px`, background: 'linear-gradient(180deg,#00D9FF,#CC00FF)', opacity: d.count === 0 ? 0.15 : 1 }} />
                    <span className="text-xs text-gray-700">{new Date(d.date).toLocaleDateString('es-ES', { weekday: 'short' })}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="neon-card rounded-2xl p-5">
              ...
              {templateBreakdown.length === 0 ? <p className="text-gray-600 text-sm">Sin datos aún</p> : (
                  ...
                  <li key={u.id} className="py-2" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    {confirmDeleteId === u.id ? (
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <p className="text-xs text-red-400">¿Eliminar <strong>{u.email}</strong>?</p>
                        <div className="flex gap-2">
                          <button onClick={() => handleDeleteUser(u.id)} disabled={deleting === u.id} className="text-xs font-bold px-3 py-1 rounded-lg" style={{ background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.4)', color: '#f87171' }}>{deleting === u.id ? '...' : 'Sí, eliminar'}</button>
                          <button onClick={() => setConfirmDeleteId(null)} className="text-xs px-3 py-1 rounded-lg" style={{ background: 'rgba(255,255,255,0.05)', color: '#6b7280' }}>Cancelar</button>
                        </div>
                      </div>
                    ) : (
                      <div className="flex items-center justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-gray-600">{new Date(u.createdAt).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' })} · {u.generationCount} gen.</p>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          {u.isPro ? <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: 'linear-gradient(135deg,#fbbf24,#f97316)', color: '#000' }}>PRO</span> : <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)', color: '#6b7280' }}>Free</span>}
                          {u.email !== ADMIN_EMAIL && <button onClick={() => setConfirmDeleteId(u.id)} className="text-xs px-2 py-1 rounded-lg" style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', color: '#f87171' }}>Eliminar</button>}
                        </div>
                      </div>
                    )}
                  </li>
              ...
              <ul className="space-y-2">
                {recentGenerations.map(g => (
                  <li key={g.id} className="flex items-center justify-between py-2" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    <div className="min-w-0">
                      <p className="text-sm text-white truncate">{g.userEmail}</p>
                      <p className="text-xs text-gray-600">{TEMPLATE_LABELS[g.template] ?? g.template} · {g.tokensUsed} tokens</p>
                    </div>
                    <span className="text-xs text-gray-600 shrink-0 ml-4">{new Date(g.createdAt).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="neon-card rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              <div>
                <p className="text-sm font-semibold text-white">Feedback de usuarios</p>
                <p className="text-xs text-gray-600 mt-0.5">Valoraciones enviadas tras 3 días de registro</p>
              </div>
              <div className="flex items-center gap-4 text-xs">
                ...
                <span style={{ color: '#facc15' }}>{feedbackPending} pendientes de enviar</span>
                <span className="text-gray-600">·</span>
                <span style={{ color: '#4ade80' }}>{feedbacks.length} recibidos</span>
              </div>
            </div>
            {feedbacksLoading ? (
              <p className="text-gray-600 text-sm">Cargando...</p>
            ) : feedbacks.length === 0 ? (
              <p className="text-gray-600 text-sm">Aún no hay feedback recibido. Los emails se envían a las 19:00 UTC a usuarios con 3+ días.</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {feedbacks.map(f => (
                  <div key={f.id} className="rounded-xl p-4" style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(255,255,255,0.06)' }}>
                    <div className="flex items-center justify-between mb-2">
                      <Stars rating={f.rating ?? 0} />
                      <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: 'rgba(255,255,255,0.05)', color: '#6b7280' }}>{f.lang.toUpperCase()}</span>
                    </div>
                    <p className="text-xs text-white/70 mb-2 leading-relaxed min-h-[2rem]">{f.comment || <span className="text-white/30 italic">Sin comentario</span>}</p>
                    <p className="text-xs text-gray-600 truncate">{f.user.email}</p>
                    <p className="text-xs text-gray-700 mt-0.5">{f.submittedAt ? new Date(f.submittedAt).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }) : '—'}</p>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="neon-card rounded-2xl p-5">
            ...
            {reviewsLoading ? <p className="text-gray-600 text-sm">Cargando...</p> : reviews.length === 0 ? <p className="text-gray-600 text-sm">No hay reseñas aún.</p> : (
                    ...
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <div className="flex gap-0.5">{[1,2,3,4,5].map(s => <FaStar key={s} size={11} style={{ color: s <= r.rating ? '#facc15' : 'rgba(255,255,255,0.15)' }} />)}</div>
                          <span className="text-xs text-gray-500 truncate">{r.user.email}</span>
                          <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: r.status === 'approved' ? 'rgba(74,222,128,0.1)' : r.status === 'rejected' ? 'rgba(248,113,113,0.1)' : 'rgba(250,204,21,0.1)', color: r.status === 'approved' ? '#4ade80' : r.status === 'rejected' ? '#f87171' : '#facc15', border: `1px solid ${r.status === 'approved' ? 'rgba(74,222,128,0.3)' : r.status === 'rejected' ? 'rgba(248,113,113,0.3)' : 'rgba(250,204,21,0.3)'}` }}>
                      ...
                      <div className="flex gap-2 shrink-0">
                        {r.status !== 'approved' && <button onClick={() => handleModerateReview(r.id, 'approved')} disabled={moderating === r.id} className="text-xs font-semibold px-3 py-1.5 rounded-lg disabled:opacity-50" style={{ background: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.3)', color: '#4ade80' }}>Aprobar</button>}
                        {r.status !== 'rejected' && <button onClick={() => handleModerateReview(r.id, 'rejected')} disabled={moderating === r.id} className="text-xs font-semibold px-3 py-1.5 rounded-lg disabled:opacity-50" style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.3)', color: '#f87171' }}>Rechazar</button>}
                      </div>
                    </div>
                  </li>
                ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="neon-card rounded-2xl p-5">
              ...
              <form onSubmit={handleCreateUser} className="space-y-3">
                <input type="email" required placeholder="Email" value={createEmail} onChange={e => setCreateEmail(e.target.value)} className="w-full bg-black/40 border border-white/10 text-white text-sm px-3 py-2 rounded-lg focus:outline-none focus:border-white/30" />
                <input type="text" placeholder="Nombre (opcional)" value={createName} onChange={e => setCreateName(e.target.value)} className="w-full bg-black/40 border border-white/10 text-white text-sm px-3 py-2 rounded-lg focus:outline-none focus:border-white/30" />
                <input type="password" required placeholder="Contraseña" value={createPassword} onChange={e => setCreatePassword(e.target.value)} className="w-full bg-black/40 border border-white/10 text-white text-sm px-3 py-2 rounded-lg focus:outline-none focus:border-white/30" />
                <button type="submit" disabled={creating} className="w-full text-sm font-semibold py-2 rounded-lg transition disabled:opacity-50" style={{ background: 'rgba(0,217,255,0.1)', border: '1px solid rgba(0,217,255,0.3)', color: '#00D9FF' }}>{creating ? 'Creando...' : 'Crear usuario'}</button>
                {createMsg && <p className="text-xs" style={{ color: createMsg.ok ? '#4ade80' : '#f87171' }}>{createMsg.text}</p>}
              </form>
            </div>

            <div className="neon-card rounded-2xl p-5">
              <p className="text-sm font-semibold text-white mb-1">Activar plan Pro</p>
              <p className="text-xs text-gray-600 mb-4">Asigna Pro manualmente sin pasar por Stripe.</p>
              <form onSubmit={handleGrantPro} className="space-y-3">
                <input type="email" required placeholder="Email del usuario" value={grantEmail} onChange={e => setGrantEmail(e.target.value)} className="w-full bg-black/40 border border-white/10 text-white text-sm px-3 py-2 rounded-lg focus:outline-none focus:border-white/30" />
                <button type="submit" disabled={granting} className="w-full text-sm font-semibold py-2 rounded-lg transition disabled:opacity-50" style={{ background: 'rgba(249,115,22,0.1)', border: '1px solid rgba(249,115,22,0.3)', color: '#fb923c' }}>{granting ? 'Activando...' : 'Activar Pro ★'}</button>
                {grantMsg && <p className="text-xs" style={{ color: grantMsg.ok ? '#4ade80' : '#f87171' }}>{grantMsg.text}</p>}
              </form>
            </div>
          </div>
