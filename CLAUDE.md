...
- **Dos repos, sin solapamiento (desde 2026-09-13)**:
  - **Web** → `C:/Users/jimen/youtube-gpt/youtube-gpt` = `javibel/youtube-gpt` (PÚBLICO). Se despliega en Vercel **automáticamente al hacer push a `main`**.
  - **Agente** → `C:/Users/jimen/youtube-gpt/local-agent` = `javibel/ytubviral-agent` (PRIVADO). Corre en PM2 desde disco, no se despliega. Todo commit de cambios del agente va **solo** aquí: `git -C C:/Users/jimen/youtube-gpt/local-agent ...`.
  - `youtube-gpt/local-agent` es un **junction** a la carpeta del agente. Está en `.gitignore` y **ya NO se versiona en el repo web** (antes sí: el código del agente acababa publicado, con una clave de Tumblr incluida). **No volver a añadirlo ni "sincronizar" nada del agente en el repo web.** ⚠️ Nunca `git rm -r local-agent` sin `--cached`: borraría el agente real a través del junction.
  - El historial del repo web anterior al 13/09 sí contiene ficheros de `local-agent/`: para buscar historia antigua del agente, mirar en ambos repos antes de dar algo por perdido (29/08/2026 se dio por perdido un fix que estaba intacto en el otro repo).
